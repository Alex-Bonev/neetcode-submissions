...
class Solution:
    def minWindow(self, s: str, t: str) -> str:
        ...
        right = 0
        t = Counter(t)
        # this is going to keep track of how many chars are good
        threshold = 0
        ...
        while(threshold < len(t) and right < len(s)):
            if s[right] in t:
                seen[s[right]]+=1
                if (seen[s[right]] == t[s[right]]):
                    # this conditional means we just got there
                    threshold+=1
            right += 1
        ...
        while (right < len(s)):
            ...
            if s[left] in t:
                seen[s[left]] -= 1
                if (seen[s[left]] == t[s[left]]-1):
                    # this means we just dropped it
                    threshold -= 1
            ...
            if s[right-1] in t:
                seen[s[right-1]] += 1
                if (seen[s[right-1]] == t[s[right-1]]):
                    threshold += 1
                while(threshold >= len(t)):
                    if s[left] in t:
                        if seen[s[left]]-1 < t[s[left]]:
                            break
                        seen[s[left]] -= 1
